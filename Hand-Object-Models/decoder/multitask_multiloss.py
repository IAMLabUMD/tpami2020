...
def loss(hypes, decoded_logits, labels):
    ...
    with tf.name_scope('loss'):
        ...
        total_loss = loss_output * head[0] + loss_output2 * head[1] + weight_loss
...
def evaluation(hypes, images, labels, decoded_logits, losses, global_step):
    ...
    if hypes['arch']['output'] == "softmax":
        ...
        eval_list.append(('Hand Acc. ', (tn+tp)/(tn + fn + tp + fp)))
        eval_list.append(('Hand loss', losses['loss']))
        ...
        eval_list.append(('Local Acc. ', (tn+tp)/(tn + fn + tp + fp)))
        eval_list.append(('Local loss', losses['loss2']))
