...
def _load_gt_file(hypes, data_file=None):
    ...
    for epoche in itertools.count():
        ...
        for file in files:
            ...
            yield image, hand_gt_image, local_gt_image
...
def _make_data_gen(hypes, phase, data_dir):
    ...
    for image, hand_gt_image, local_gt_image in data:
        if phase == 'val':
            yield image, get_gt_image(hypes, hand_gt_image, local_gt_image)
        elif phase == 'train':
            yield jitter_input(hypes, image, hand_gt_image, local_gt_image)
...
def jitter_input(hypes, image, hand_gt_image, local_gt_image):
    jitter = hypes['jitter']
