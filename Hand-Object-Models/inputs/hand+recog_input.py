...
def _load_gt_file(hypes, data_file=None):
    ...
    for epoche in itertools.count():
        shuffle(files)
        for file in files:
            image_file, local_gt_image_file = file.split("\t")
            # get the hand mask file (Objects -> Hands)
            if "BigObjects" in local_gt_image_file:
                hand_gt_image_file = local_gt_image_file.replace("/BigObjects/", "/Masks/", 1)
            else:
                hand_gt_image_file = local_gt_image_file.replace("/Objects/", "/Masks/", 1)

            if "GTEA" in hand_gt_image_file:
                hand_gt_image_file = hand_gt_image_file.replace(".jpg", ".png", 1)

            image_file = os.path.join(base_path, image_file)
            assert os.path.exists(image_file), \
                "File does not exist: %s" % image_file
            hand_gt_image_file = os.path.join(base_path, hand_gt_image_file)
            assert os.path.exists(hand_gt_image_file), \
                "File does not exist: %s" % hand_gt_image_file
            local_gt_image_file = os.path.join(base_path, local_gt_image_file)
            assert os.path.exists(local_gt_image_file), \
                "File does not exist: %s" % local_gt_image_file

            # image = scipy.misc.imread(image_file, mode='RGB')
            image = cv2.imread(image_file, cv2.IMREAD_COLOR)
            hand_gt_image = cv2.imread(hand_gt_image_file, cv2.IMREAD_COLOR)
            local_gt_image = cv2.imread(local_gt_image_file, cv2.IMREAD_COLOR)
            # get the label of image here
            gt_label = get_gt_class(hypes, base_path, image_file)

            yield image, hand_gt_image, local_gt_image, gt_label


def _get_center(array):
    indices = np.argwhere(array == True)
    if indices.size == 0:
        return -1.0, -1.0
    return np.median(indices, axis=0)
...
def _make_data_gen(hypes, phase, data_dir):
    ...
    train_label = hypes['data']['train_label']
    test_label = hypes['data']['test_label']

    data_file = os.path.join(data_dir, data_file)
    data = _load_gt_file(hypes, data_file)
    for image, hand_gt_image, local_gt_image, gt_label in data:
        if phase == 'val':
            yield image, get_gt_image(hypes, hand_gt_image, local_gt_image), gt_label
        elif phase == 'train':
            yield jitter_input(hypes, image, hand_gt_image, local_gt_image, gt_label)
            yield jitter_input(hypes, np.fliplr(image), 
                                np.fliplr(hand_gt_image),
                                np.fliplr(local_gt_image),
                                gt_label)
...
"""
Convert RGB images of ground-truths into annotation
"""        
def get_gt_image(hypes, hand_gt_image, local_gt_image):
    """
...
def jitter_input(hypes, image, hand_gt_image, local_gt_image, gt_class):
    jitter = hypes['jitter']
    ...
    if jitter['random_crop'] and crop_chance > random.random():
        ...
        crop_chance = jitter['crop_chance']
        image, hand_gt_image, local_gt_image = random_crop_soft(
                                                image,
                                                hand_gt_image,
                                                local_gt_image,
                                                max_crop)
    ...
    if jitter['crop_patch']:
        ...
        image, hand_gt_image, local_gt_image = random_crop(
                                                image,
                                                hand_gt_image,
                                                local_gt_image,
                                                patch_height,
    ...
    # now get the groun-truth annotation
    gt_image = get_gt_image(hypes, hand_gt_image, local_gt_image)

    assert(image.shape[:-1] == gt_image.shape[:-1])

    return image, gt_image, gt_class


def random_crop(image, hand_gt_image, local_gt_image, height, width):
    ...
    hand_gt_image = hand_gt_image[offset_x:offset_x+height, offset_y:offset_y+width]
    local_gt_image = local_gt_image[offset_x:offset_x+height, offset_y:offset_y+width]
...
def random_crop_soft(image, hand_gt_image, local_gt_image, max_crop):
    offset_x = random.randint(1, max_crop)
    offset_y = random.randint(1, max_crop)

    if random.random() > 0.5:
        image = image[offset_x:, offset_y:, :]
        hand_gt_image = hand_gt_image[offset_x:, offset_y:, :]
        local_gt_image = local_gt_image[offset_x:, offset_y:, :]
    else:
        image = image[:-offset_x, :-offset_y, :]
        hand_gt_image = hand_gt_image[:-offset_x, :-offset_y, :]
        local_gt_image = local_gt_image[:-offset_x, :-offset_y, :]
...
def start_enqueuing_threads(hypes, q, phase, sess):
    ...
    if hypes['arch']['output'] == "softmax":
        label_pl = tf.placeholder(tf.float32, [None, None, num_classes])
    elif hypes['arch']['output'] == "sigmoid" \
            or hypes['arch']['output'] == "regress":
        label_pl = tf.placeholder(tf.float32, [None, None, 2])
    class_pl = tf.placeholder(tf.float32, [num_obj_classes])

    data_dir = hypes['dirs']['data_dir']

    def make_feed(data):
        image, label, cl = data
        return {image_pl: image, label_pl: label, class_pl: cl}
    ...
    enqueue_op = q.enqueue((image_pl, label_pl, class_pl))
...
def _read_processed_image(hypes, q, phase):
    ...
    obj_class = tf.expand_dims(obj_class, 0)

    return image, label, obj_class
...
def inputs(hypes, q, phase):
    ...
    if phase == 'val':
        image, label, obj_class = q.dequeue()
        image = tf.expand_dims(image, 0)
        label = tf.expand_dims(label, 0)
        obj_class = tf.expand_dims(obj_class, 0)
        return image, label, obj_class
    ...
    if not shape_known:
        ...
        obj_class.set_shape([num_obj_classes])
        ...
        image, label, obj_class = q.dequeue_many(hypes['solver']['batch_size'])
    ...
    hand = tf.expand_dims(tf.to_float(label[:, :, :, 1]), 3)
    tf.summary.image(tensor_name + '/hand_gt_image', hand)

    obj = tf.expand_dims(tf.to_float(label[:, :, :, 2]), 3)
    tf.summary.image(tensor_name + '/local_gt_image', obj)
...
def main():
    """main."""
    with open('../hypes/TOR_local.json', 'r') as f:
        hypes = json.load(f)
    ...
    image_batch, label_batch, class_batch = inputs(hypes, q['train'], 'train')
