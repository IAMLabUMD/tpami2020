...
def _load_gt_file(hypes, data_file=None):
    ...
    for epoche in itertools.count():
        ...
        for file in files:
            ...
            # print("{} {}".format(image_file, gt_image_file))

            yield image, gt_image, gt_label
...
def _make_data_gen(hypes, phase, data_dir):
    ...
    for image, gt_image, gt_label in data:
        if phase == 'val':
            yield image, gt_image, gt_label
        elif phase == "train":
            yield jitter_input(hypes, image, gt_image, gt_label)
            yield jitter_input(hypes, np.fliplr(image), np.fliplr(gt_image), gt_label)
...
"""        
def get_gt_image(hypes, gt_image):
    """
...
def jitter_input(hypes, image, gt_image, gt_class):
    ...
    # now get the ground-truth annotation
    gt_image = get_gt_image(hypes, gt_image)
...
def resize_label_image(image, gt_image, image_height, image_width, gt_class):
    """
    ...
    image = cv2.resize(image, (image_width, image_height), \
            interpolation = cv2.INTER_CUBIC)
    gt_image = cv2.resize(gt_image, (image_width, image_height), \
            interpolation = cv2.INTER_NEAREST)
...
def main():
    """main."""
    with open('../hypes/TOR_hand+recog_TEgO_softmax_big.json', 'r') as f:
        hypes = json.load(f)
    ...
    # image_batch, label_batch, class_batch = inputs(hypes, q, 'train', data_dir)
